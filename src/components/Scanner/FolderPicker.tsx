...
export function FolderPicker({ onNavigate, onGamesSaved }: FolderPickerProps) {
  ...
  useEffect(() => {
    if (!scanning && consoleLogs.length > 0) setConsoleMinimized(true);
  ...
  const handleScanAll = async () => {
    ...
    try {
      ...
      await new Promise<void>((resolve, reject) => {
        unlisteners.push(() => listen("scan:complete", () => resolve()).then((fn) => fn));
        unlisteners.push(() => listen("scan:cancelled", () => resolve()).then((fn) => fn));
        invoke("scan_folders_smart", { paths: folders.map((f) => f.path) }).catch(reject);
      });
      setShowResults(true);
    } catch {} finally {
      ...
      {(showResults || results.length > 0) && !scanning && (
        <div className="space-y-4 theme-border border-t pt-4">
          ...
          {results.length === 0 ? (
            ...
            <ScanResults results={results} onSave={handleSave} onResultsChange={setResults}
              onCreateExclusion={() => {}} />
          )}
        </div>
      )}
