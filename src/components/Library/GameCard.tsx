...
const GameCard: React.FC<GameCardProps> = ({ game, onClick, viewMode, onFilter, showQuickAssign, onQuickAssign }) => {
  ...
  const renderStatusBadge = () => {
    ...
    return (
      <button
        ...
        className={`absolute top-2 left-2 px-2 py-1 rounded-full text-xs text-white ${statusColors[completion_status] || 'bg-gray-600'} hover:opacity-80 transition-opacity cursor-pointer select-none z-10`}
      >
        {label}
      </button>
    );
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    // Only navigate if the event was triggered directly on the card
    // This prevents bubbling from child interactive elements like tags
