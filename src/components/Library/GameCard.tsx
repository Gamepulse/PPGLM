      ...
      <div className="flex flex-wrap gap-1 mt-2 pointer-events-auto">
        {displayedTags.map((tag) => (
          <button
            ...
            className={`relative z-20 px-2 py-0.5 text-xs rounded-full text-white ${getCategoryColor(tag.category)} hover:opacity-80 hover:scale-110 hover:shadow-md transition-all cursor-pointer select-none border border-transparent hover:border-white/30 pointer-events-auto focus-visible:ring-2 ring-white`}
      ...
      <div className="flex flex-wrap gap-1 mt-1">
        {allMetadata.map((item) => (
          <button
            ...
            className="px-1.5 py-0.5 text-xs rounded bg-gray-700/50 theme-text-muted hover:bg-indigo-600/30 transition-colors cursor-pointer select-none focus-visible:ring-2 ring-indigo-500"
            aria-label={`${t(item.type === 'mode' ? 'gameMode' : item.type === 'perspective' ? 'playerPerspective' : item.type as any)}: ${item.name}`}
          >
            {item.name}
          </button>
        ))}
  ...
  const renderStatusBadge = () => {
    ...
    return (
      <button
        ...
        className={`absolute top-2 left-2 px-2 py-1 rounded-full text-xs text-white ${statusColors[completion_status] || 'bg-gray-600'} hover:opacity-80 transition-opacity cursor-pointer select-none z-10 focus-visible:ring-2 ring-white`}
          ...
          <h3 className="theme-text-primary font-semibold text-lg mb-2 truncate flex items-center gap-2">
            ...
            {showQuickAssign && (
              <button
                ...
                className="ml-auto text-xs px-2 py-1 bg-orange-500 hover:bg-orange-600 text-white rounded transition-colors focus-visible:ring-2 ring-orange-400"
        ...
        <div className="relative h-24 bg-gray-800">
          ...
          {completion_status && completion_status !== 'not_started' && (
            <button
              ...
              } hover:opacity-80 transition-opacity cursor-pointer focus-visible:ring-1 ring-white`}
              aria-label={`${t('completionStatus')}: ${
                completion_status === 'not_started' ? t('notStarted') :
                completion_status === 'playing' ? t('playing') :
                completion_status === 'completed' ? t('completed') :
                completion_status === 'dropped' ? t('dropped') :
                completion_status === 'wishlist' ? t('wishlist') : completion_status
              }`}
            >
              {completion_status === 'not_started' ? t('notStarted') :
               completion_status === 'playing' ? t('playing') :
          ...
          {showQuickAssign && (
            <button
              ...
              className="absolute bottom-1 left-1 w-5 h-5 flex items-center justify-center text-[10px] bg-orange-500 hover:bg-orange-600 text-white rounded transition-colors z-10 focus-visible:ring-1 ring-orange-400"
              title={t('quickAssignPlatform')}
              aria-label={t('quickAssignPlatform')}
            >
              <span aria-hidden="true">🎮</span>
          ...
          <h3 className="theme-text-primary font-semibold text-lg mb-2 flex items-center gap-2">
            ...
            {showQuickAssign && (
              <button
                ...
                className="ml-2 text-xs px-2 py-1 bg-orange-500 hover:bg-orange-600 text-white rounded transition-colors focus-visible:ring-2 ring-orange-400"
                title={t('quickAssignPlatform')}
                aria-label={t('quickAssignPlatform')}
              >
                <span aria-hidden="true">🎮</span>
